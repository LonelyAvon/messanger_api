...
from app.api.schemas.chat import ChatCreate, ChatInfo, ChatPreview, ChatRead
...
class ChatService:
    ...
    async def create(self, chat: ChatCreate) -> ChatRead:
        created_chat: ChatRead = await ChatRepository(self.session).create(type=chat.type, name=chat.name)
    ...
    async def get_chats(self, user_id: UUID) -> list[ChatPreview]:
        ...
        for chat in chats:
            chat_preview: ChatPreview = ChatPreview(id=chat.id, chat_name=chat.chat_name, photo=chat.photo, type=chat.type)

            last_message_chat = await ChatRepository(self.session).get_last_message_by_chat_id(chat_id=chat.id)
    ...
    async def get_messages(self, chat_id: UUID):
        chat = await ChatRepository(self.session).get_chat(chat_id=chat_id)
        messages: list[ChatMessage] = await ChatMessageRepository(self.session).get_chat_messages(chat_id=chat.id)
        chat_info: ChatInfo = ChatInfo(id=chat.id, chat_name=chat.chat_name, messages=messages)
        return chat_info