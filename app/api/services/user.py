...
from app.api.schemas.user import ChatUser, UserRead, UserUpdate
...
class UserService:
    ...
    async def update_user(self, user_id, user: UserUpdate) -> UserRead:
        user: UserRead = await UserRepository(self.session).update_one(
    ...
    async def reset_password_by_email(self, user: UserRead):
        ...
        await self.send_message(
            user.email, "Восстановление пароля", f"Ваш новый пароль: {new_password}"
        )
        user: UserRead = await UserRepository(self.session).update_password(
            new_password
        )
        await UserRepository(self.session).commit()
        return user

    async def reset_password_by_old_password(
        self, user: UserRead, old_password, new_password
    ):
    ...
    async def verified_email_by_token(self, token: str) -> UserRead:
        ...
        user: UserRead = await UserRepository(self.session).update_one(
            user.id, is_verified_email=True
        )
        await UserRepository(self.session).commit()
        return user
