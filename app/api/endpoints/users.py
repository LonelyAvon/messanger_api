...
from app.db.db import async_session, get_session
...
@router.put(
    "/update", response_model=UserRead, summary="Обновление информации о пользователе"
)
...
@router.post("/photo", response_model=UserRead, summary="Загрузка фото профиля")
...
@router.post("/email/verify", summary="Проверка почты")
async def email_send_verified_message(
    request: Request,
    user: UserRead = Depends(get_current_user),
    session: AsyncSession = Depends(get_session),
):
    ...
    await UserService(session).send_message(
        user.email,
        "Ссылка",
        f"Ваша ссылка для потдверждения почты: \n{settings.get_domen}/user/email/{token}",
    )
...
@router.post("/password/reset/email", summary="Сброс пароля по почте")
...
@router.post("/password/reset/old_password", summary="Сброс пароля по старому паролю")
async def password_reset_by_old_password(
    old_password: str,
    new_password: str,
    user: UserRead = Depends(get_current_user),
    session: AsyncSession = Depends(get_session),
):
    user: UserRead = await UserService(session).reset_password_by_old_password(
